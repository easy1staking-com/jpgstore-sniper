...
//        var assetType = AssetType.fromUnit("a706fc87764cde4ac018c38bf61630c1065932db49e6f495be3b29f8436f636f4c6f636f426c756550616c6d53314e4654313737");
        ...
        log.info("listingDatum: {}", OBJECT_MAPPER.writeValueAsString(listingDatum));
        ...
        log.info("utxo hash: {}", utxo.getDataHash());
        ...
        var walletUtxos = bfBackendService.getUtxoService().getUtxos(account.baseAddress(), 100, 1).getValue()
                .stream()
                .filter(foo -> foo.getAmount().getFirst().getQuantity().compareTo(BigInteger.valueOf(20_000_00L)) > 0)
                .toList()
                .getFirst();
...
//        var jpgStoreFees = Rational.from(bar).multiply(Rational.from(25L, 975L)).floor();
        ...
        var tx = new ScriptTx()
                .collectFrom(List.of(walletUtxos))
...
//                .attachSpendingValidator(jpgStoreContract)
