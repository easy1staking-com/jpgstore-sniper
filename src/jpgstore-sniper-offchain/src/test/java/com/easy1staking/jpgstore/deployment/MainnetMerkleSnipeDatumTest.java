...
@Slf4j
@Tag("deployment")
public class MainnetMerkleSnipeDatumTest extends AbstractTest {
    ...
    public void mintSnipeTest() throws Exception {
        ...
        var merkleSnipeBytesOpt = plutusService.getContractCode("snipe.merkle_snipe.mint");
        if (merkleSnipeBytesOpt.isEmpty()) {
            Assertions.fail();
        }

        var merkleSnipeBytes = merkleSnipeBytesOpt.get();

        var merkleSnipeParameters = ListPlutusData.of(BytesPlutusData.of(settingsContract.getScriptHashBytes()));

        var merkleSnipeContract = PlutusBlueprintUtil.getPlutusScriptFromCompiledCode(
                AikenScriptUtil.applyParamToScript(merkleSnipeParameters, merkleSnipeBytes),
                PlutusVersion.v3
        );
        ...
        var snipeAddress = AddressProvider.getBaseAddress(Credential.fromScript(merkleSnipeContract.getScriptHash()),
        ...
        var lockedAmount = Value.fromCoin(BigInteger.valueOf(12_000_000L))
                .add(Value.from(merkleSnipeContract.getPolicyId(), "0x", BigInteger.ONE));
        ...
        var tx = new ScriptTx()
                ...
                .mintAsset(merkleSnipeContract, Asset.builder().value(BigInteger.ONE).build(), markleSnipeMintRedeemer)
                .payToContract(snipeAddress.getAddress(), ValueUtil.toAmountList(lockedAmount), snipe.toPlutusData())
